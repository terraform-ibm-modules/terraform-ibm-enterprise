...
- `existing_access_groups` (optional): (List) A list of names of existing access groups to which the user will be assigned. If empty list is passed then user will only be invited to the sub account and will not be assigned to any access group.
...
[
  {
    ...
    existing_access_groups = ["Developers", "Auditors"]
  },
  {
    email                   = "user2@example.com"
    existing_access_groups = ["Support Team"]
  }
]
```
